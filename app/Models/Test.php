...
class Test extends Model
{
    ...
    public function users(){
        return $this->belongsToMany(\App\Models\User::class, 'question_user')->withPivot('correct','question_answered','answered_date', 'attempts', 'question_id','kudos')->withTimestamps();
    }

    public function testee(){
        return $this->belongsToMany(\App\Models\User::class, 'test_user')->withPivot('test_completed', 'completed_date', 'result', 'attempts', 'kudos')->withTimestamps();
    }
    ...
    public function results(){
        return $this->morphMany(\App\Models\Result::class, 'assessment');
    }

    public function activities(){
        return $this->morphMany(\App\Models\Activity::class, 'classwork');
    }
    ...
    protected function getAdaptiveQuestions($user, $level){
        ...
        if (count($unansweredQuestions) < $numToField ){
            $unpassedQuestions = \App\Models\Question::whereSkillId($user->skilluser()->wherePivot('skill_passed', false)->pluck('id'))->take($numToField - count($unansweredQuestions))->get();
        }

        // 2. Get unattempted skills
        if (count($unansweredQuestions) + count($unpassedQuestions) < $numToField){
            $houseIds = $user->validEnrolment(\App\Models\Course::where('course', 'LIKE', '%Math%')->pluck('house_id'));
            $trackIds = \App\Models\House_Track::whereIn('house_id', $houseIds)->pluck('track_id');
            $skillIds = \App\Models\Skill_Track::whereIn('track_id', $trackIds)->pluck('skill_id');
            $unattemptedQuestions = \App\Models\Question::whereIn('skill_id', $skillIds) ->inRandomOrder()->take($numToField - count($unansweredQuestions) - count($unpassedQuestions))->get();
        }
    ...
    public function fieldQuestions($user) {
        ...
        if (count($unansweredQuestions) < $numToField) {
            ...
            if ($this->diagnostic) {
                $newQuestions = $this->getDiagnosticQuestionsbyLevel($user, \App\Models\Level::find($level));
            } else {
