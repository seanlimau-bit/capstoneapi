...
class SkillController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.skills.index', compact('skills', 'maps'));
    }
    ...
    public function linkVideo(Request $request, Skill $skill)
    {
        ...
        $video = Video::where('video_link', $data['video_link'])->firstOrFail();

        $sortOrder = $data['sort_order'] ?? (
            (\DB::table('skill_video')->where('skill_id', $skill->id)->max('sort_order')) + 1
        );

        // If status_id missing or null, use 1
        ...
        return response()->json([
            'success' => true,
            'video' => ['id' => $video->id, 'video_link' => $video->video_link],
        ], 201);
    }
