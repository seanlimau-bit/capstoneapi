            ...
            <div class="modal-body">
                ...
                <div class="mb-4">
                    <h6 class="fw-bold text-muted mb-3">
                        <i class="fas fa-link me-1"></i>CURRENTLY ASSOCIATED TRACKS (<?php echo e($skill->tracks->count()); ?>)
                    </h6>
                    <?php if($skill->tracks->count() > 0): ?>
                        <div id="currentTracks" class="row">
                            ...
                            <div class="col-md-6 mb-2" data-track-id="<?php echo e($track->id); ?>">
                                <div class="card border-primary">
                                    <div class="card-body p-3">
                                        <div class="d-flex justify-content-between align-items-start">
                                            <div class="flex-grow-1">
                                                <h6 class="card-title mb-1"><?php echo e($track->track); ?></h6>
                                                <p class="card-text small text-muted mb-1"><?php echo e(strlen($track->description) > 50 ? substr($track->description, 0, 50) . '...' : $track->description); ?></p>
                                                <?php if($track->level): ?>
                                                    <span class="badge bg-info">Level <?php echo e($track->level->level); ?></span>
                                                <?php endif; ?>
                                            </div>
                                            <button class="btn btn-outline-danger btn-sm" 
                                                    onclick="removeTrackFromModal(<?php echo e($track->id); ?>)"
                                                    title="Remove track">
...
let currentSkillTracks = <?php echo json_encode($skill->tracks->pluck('id')->toArray(), 15, 512) ?>;
...
function addTrackToSkill(trackId, trackName) {
    fetch(`/admin/skills/<?php echo e($skill->id); ?>/tracks`, {
        method: 'POST',
        headers: {
...
function removeTrackFromSkill(trackId) {
    fetch(`/admin/skills/<?php echo e($skill->id); ?>/tracks/${trackId}`, {
        method: 'DELETE',
        headers: {
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
            'Accept': 'application/json'
...
</script><?php /**PATH C:\allgifted\mathapi11v2\resources\views\admin\skills\modals\manage-tracks.blade.php ENDPATH**/ ?>