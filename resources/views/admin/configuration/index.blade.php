                            ...
                            <table class="table table-hover" id="difficultiesTable">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="sortable" data-col="2">Short Description</th>
                                        ...
                                        <tr data-id="{{ $difficulty->id }}" data-status-id="{{ $difficulty->status_id ?? '' }}"
                                            ...
                                            <td class="click-edit">
                                                <span class="view-mode status-badge"
                                                    data-status-id="{{ $difficulty->status_id ?? '' }}"></span>
                                                <select class="form-select form-select-sm edit-mode d-none inline-edit"
                                ...
                                <tbody>
                                    @foreach($types as $type)
                                        <tr data-id="{{ $type->id }}" data-status-id="{{ $type->status_id ?? '' }}"
                                            ...
                                            <td class="click-edit">
                                                <span class="view-mode status-badge"
                                                    data-status-id="{{ $type->status_id ?? '' }}"></span>
                                                <select class="form-select form-select-sm edit-mode d-none inline-edit"
                                                    data-field="status_id"></select>
                                            </td>
                                            <td>
                                ...
                                <tbody>
                                    @foreach($levels as $level)
                                        <tr data-id="{{ $level->id }}" data-status-id="{{ $level->status_id ?? '' }}"
                                            ...
                                            <td class="click-edit">
                                                <span class="view-mode status-badge"
                                                    data-status-id="{{ $level->status_id ?? '' }}"></span>
                                                <select class="form-select form-select-sm edit-mode d-none inline-edit"
                                                    data-field="status_id"></select>
                                            </td>
                                            <td>
...
@push('scripts')
    <script>
        ...
        $(document).ready(function () {
            ...
            // Hydrate status selects and badges in existing rows
            function hydrateRowStatusSelects(scope) {
                (scope || $(document)).find('tr.data-row').each(function () {
                    const $tr = $(this);
                    const current = String($tr.attr('data-status-id') ?? '');
                    const $select = $tr.find('select.inline-edit[data-field="status_id"]');
                    if ($select.length) $select.html(renderStatusOptions(current));
                    const $badge = $tr.find('span.status-badge');
                    if ($badge.length) {
                        const sid = $badge.attr('data-status-id') || current;
                        $badge.html(renderStatusBadge(sid));
                    }
                });
            }

            hydrateRowStatusSelects();
            ...
            function addNewRowToTable(tableType, data) {
                ...
                // hydrate just the inserted row
                hydrateRowStatusSelects($tbody.find('tr.data-row').first());
            }
            ...
            function enterEditMode($row) {
                ...
                // focus first input
                const $first = $row.find('.inline-edit').first();
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        $row.find('.inline-edit').each(function () {
                            ...
                            if (field === 'start_maxile_level' || field === 'end_maxile_level') {
                                const startVal = $row.find('[data-field="start_maxile_level"]').val();
                                const endVal = $row.find('[data-field="end_maxile_level"]').val();
                                $row.find('.maxile-span').text(`${startVal} - ${endVal}`);
                            } else if (field === 'status_id') {
                                $row.attr('data-status-id', newValue);
                                // badge element may be empty until hydrated
                                const $badgeSpan = $td.find('.status-badge');
                                if ($badgeSpan.length) $badgeSpan.html(renderStatusBadge(newValue));
                                else $view.html(renderStatusBadge(newValue));
